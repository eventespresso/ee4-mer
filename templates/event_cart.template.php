...
/**
 * @var int $total_items
 * @var string $btn_class
 * @var string $empty_cart_url
 * @var string $event_cart
 * @var string $event_cart_heading
 * @var string $event_cart_name
 * @var string $events_list_url
 * @var string $register_url
 * @var string $update_cart_url
 */
	...
	<form id="event-cart-qty-frm" action="<?php echo $register_url; ?>" method="POST">
		...
		<div class="event-cart-grand-total">
            <?php if ($total_items) { ?>
				...
				<a class="event-cart-empty-cart-lnk empty-cart-lnk event-cart-button button <?php echo $btn_class; ?>"
				   href="<?php echo $empty_cart_url; ?>">
					<span class="dashicons dashicons-trash"></span><?php echo apply_filters(
                        'FHEE__EED_Multi_Event_Registration__empty_event_cart_btn_txt',
                        sprintf(__('Empty %s', 'event_espresso'), $event_cart_name)
                    ); ?>
				</a>
			</span>
				<span class=" smaller-text">
				<a class="event-cart-update-cart-lnk update-cart-lnk event-cart-button button <?php echo $btn_class; ?>"
				   href="<?php echo $update_cart_url; ?>"
				>
					<span class="dashicons dashicons-update"></span><?php echo apply_filters(
                        'FHEE__EED_Multi_Event_Registration__update_event_cart_btn_txt',
                        sprintf(__('Update %s', 'event_espresso'), $event_cart_name)
                    ); ?>
				</a>
			</span>
            <?php } // if ( $total_items )  ?>
		...
		<div id="event-cart-whats-next-buttons" class="event-cart-whats-next-buttons">
			...
			<a class="event-cart-go-back-button event-cart-button button <?php echo $btn_class; ?>"
			   href="<?php echo $events_list_url; ?>"
			>
				<span class="dashicons dashicons-arrow-left-alt2"></span><?php echo apply_filters(
                    'FHEE__EED_Multi_Event_Registration__return_to_events_list_btn_txt',
                    __('Return to Events List', 'event_espresso')
                ); ?>
			</a>

            <?php if ($total_items) { ?>

				<a class="event-cart-register-button event-cart-button button <?php echo $btn_class; ?>"
